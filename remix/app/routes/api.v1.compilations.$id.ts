...
import {
  DeleteCompilationResponse,
  UpdateCompilationResponse,
} from "~/types/compilations";
...
export async function loader({ params }: LoaderFunctionArgs) {
  ...
  const { data, success } = retrieveCompilationRequestSchema.safeParse({
...
export async function action({
  ...
  UpdateCompilationResponse | DeleteCompilationResponse | errorResponseType
> {
  ...
  if (request.method === "PATCH") {
    ...
    const submission = parseWithZod(formData, {
      schema: updateCompilationRequestSchema,
    });
    if (submission.status !== "success") {
      return ERROR_RESPONSES.INVALID_ARGUMENT();
    }

    const compilation = await compilationService.get({
  ...
  if (request.method === "DELETE") {
    ...
    formData.append("id", params.id);
    const submission = parseWithZod(formData, {
      schema: deleteCompilationRequestSchema,
    });
    if (submission.status !== "success") {
      return ERROR_RESPONSES.INVALID_ARGUMENT();
    }
