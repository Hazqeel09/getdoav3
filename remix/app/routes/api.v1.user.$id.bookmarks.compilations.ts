...
export async function loader({ request, params }: LoaderFunctionArgs): Promise<ListCompilationsResponse | errorResponseType> {
  if (!params.id) {
  ...
  const { success } = listCompilationsParamsSchema.safeParse({ limit, page, user_id });
  if (!success) {
    return ERROR_RESPONSES.INVALID_ARGUMENT;
  }
