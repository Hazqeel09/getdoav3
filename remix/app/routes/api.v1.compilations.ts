...
export async function action({
  request,
}: ActionFunctionArgs): Promise<CreateCompilationResponse | errorResponseType> {
  ...
  const submission = parseWithZod(formData, {
    schema: createCompilationRequestSchema,
  });
  if (submission.status !== "success") {
    return ERROR_RESPONSES.INVALID_ARGUMENT();
  }

  return await compilationService.create({
    ...submission.value,
    user_id: user.id,
  });
}
